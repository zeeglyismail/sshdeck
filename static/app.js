...
function renderTree() {
  ...
  const folderRow = (f, depth) => {
    ...
    fe.oncontextmenu = e => ctxMenu(e, [
      ...
      { label: "Delete", danger: true, fn: async () => {
          const u = await api(`/api/folders/${f.id}/usage`);
          ...
          if (!choice) return;
          await api(`/api/folders/${f.id}?delete_hosts=${choice === "all"}`, { method: "DELETE" });
          loadState();
        } },
    ]);
...
/* delete confirm that explains what happens to hosts using the credential */
async function confirmCredDelete(kind, name, usageUrl) {
  const u = await api(usageUrl);
  const n = u.hosts.length;
  const msg = n
    ? `<b>${n} host${n > 1 ? "s" : ""}</b> use this ${kind}: <span class="muted mono">${u.hosts.slice(0, 8).map(esc).join(", ")}${n > 8 ? ` … +${n - 8}` : ""}</span><br><br>` +
  ...
  const c = await choose(`Delete ${kind} "${name}"?`, msg,
    [{ label: "Cancel", value: null }, { label: "Delete", value: "yes", cls: "btn-danger" }]);
  return c === "yes";
}
...
function renderIdentities() {
  ...
  for (const i of STATE.identities) {
    ...
    item.querySelector(".del").onclick = async () => {
      if (!await confirmCredDelete("identity", i.name, `/api/identities/${i.id}/usage`)) return;
...
function renderKeys() {
  ...
  for (const k of STATE.keys) {
    ...
    item.querySelector("button").onclick = async () => {
      if (!await confirmCredDelete("key", k.name, `/api/keys/${k.id}/usage`)) return;
